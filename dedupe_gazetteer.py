...
We consider two groups to show the training process and prediction process called:
- Group of control: It is used to train the machine learning system and
  to evaluate the model used.
- Group of application: It is used to predict the data matches
  for a new data set.

For the group of control,
will use the following output files, generated by running the script 'prepara_dados_grupo_controle.py':
...
These files are saved in 'arquivos/grupo_controle/dados_entrada'


For the group of application,
will use the following output files, generated by running the script 'prepara_dados_grupo_aplicacao.py':
- 'cj_canonico_pesquisadores.csv'
- 'cj_messy_autorias.csv'
These files are saved in 'arquivos/grupo_aplicacao/dados_entrada'

Note: If you want to train from scratch, delete the settings_file and the training_file
      saved in 'arquivos/dados_treinamento'
...
ARQUIVOS_TREINAMENTO = os.path.join(ARQUIVOS_DIR,'dados_treinamento')
ARQUIVOS_ENTRADA_GC = os.path.join(ARQUIVOS_GC_DIR,'dados_entrada')
ARQUIVOS_SAIDA_GC = os.path.join(ARQUIVOS_GC_DIR,'dados_saida')
ARQUIVOS_ENTRADA_GA = os.path.join(ARQUIVOS_GA_DIR,'dados_entrada')
ARQUIVOS_SAIDA_GA = os.path.join(ARQUIVOS_GA_DIR,'dados_saida')

## Grou of Control (GC): Group for the training process and model evaluation
# input files
pesquisadores_file = os.path.join(ARQUIVOS_ENTRADA_GC,'cj_canonico_pesquisadores.csv')
autorias_treinamento_file = os.path.join(ARQUIVOS_ENTRADA_GC,'cj_messy_autorias_para_treinamento.csv')
autorias_validacao_file = os.path.join(ARQUIVOS_ENTRADA_GC,'cj_messy_autorias_para_validacao.csv')
autorias_teste_file = os.path.join(ARQUIVOS_ENTRADA_GC,'cj_messy_autorias_para_teste.csv')
# output files
output_file = os.path.join(ARQUIVOS_SAIDA_GC,'gazetteer_matches_found.csv')
...
## Group of Application (GA)
# input files
ga_pesquisadores_file = os.path.join(ARQUIVOS_ENTRADA_GA,'cj_canonico_pesquisadores.csv')
ga_autorias_file = os.path.join(ARQUIVOS_ENTRADA_GA,'cj_messy_autorias.csv')
# output files
ga_output_file = os.path.join(ARQUIVOS_SAIDA_GA,'gazetteer_matches_found.csv')

## Training files
settings_file = os.path.join(ARQUIVOS_TREINAMENTO,'gazetteer_learned_settings')
training_file = os.path.join(ARQUIVOS_TREINAMENTO,'gazetteer_training.json')
...
class Gazetteer_EntityMatching:
    ...
    def predictionProcess(self, messy_set_file, output_file):

        print('\n[PREDICTION PROCESS]')

        self.messy_test_file = messy_set_file
        print('Importing messy test data ...')
        self.messy_test_d = readData(self.messy_test_file)
        print(f"Number of records from messy data (autorias): {len(self.messy_test_d)}")
    ...
    def save_prediction_result(self, output_file):
        ### saving information obtained from the prediction process

        canon_file = self.canonical_file
        messy_file = self.messy_test_file

        canonical_d = readDataToSaveResults(canon_file)
        messy_test_d = readDataToSaveResults(messy_file)
    ...
    def save_false_positives(self, false_positives_s, false_positives_file):
        ### saving information obtained from the prediction and model evaluation processes
...
if __name__ == '__main__':
    ...
    if 'controle' in sys.argv:
        ...
        if not os.path.exists(settings_file):
            print(f'There is no settings file {settings_file}')
        else:
            start_time = datetime.now()
            entity_matching = Gazetteer_EntityMatching(canonical_set_file=ga_pesquisadores_file, settings_file=settings_file, training_file='')
            print(f"Number of records from canonical data (pesquisadores unicos): {len(entity_matching.canonical_d)}")
            entity_matching.predictionProcess(ga_autorias_file, ga_output_file)
            end_time = datetime.now()
